...
let rateFilter=(rateVal)=>{
 ...
 // https://lazy-jade-goat-wear.cyclic.app//products?categoryID=64233ff9213d64eea40879d3&subCategoryID=64257abda2b34947c00d20a4
//  &subCategoryID=${subCatId}
  ...
  useEffect(() => {
    if(subCatId){
      fetch(`https://lazy-jade-goat-wear.cyclic.app//Products?categoryID=${catId}&subCategoryID=${subCatId}&instock[gte]=${stock}&price[gte]=${price}&price[lte]=${priceM}&ratings[gte]=${rate}&sort=${sort}&page=${page}&limit=4`)
      .then((res) => res.json())
      .then((json) => {
        console.log(json);
        setNumOfPage(json.paginationResult.numberOfPages);
        setProduct(json.data);
      });

    }
    else{
      fetch(`https://lazy-jade-goat-wear.cyclic.app//Products?categoryID=${catId}&instock[gte]=${stock}&price[gte]=${price}&price[lte]=${priceM}&ratings[gte]=${rate}&sort=${sort}&page=${page}&limit=6`)
      .then((res) => res.json())
      .then((json) => {
  ...
  }, [stock,price,priceM,rate,sort,page]);
        ...
        <div className="row mt-5">
          ...
          <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                      <li onClick={()=>{setSort('-price')}}><a class="dropdown-item" href="#">Heigh Price</a></li>
                      <li onClick={()=>{setSort('price')}}><a class="dropdown-item" href="#">Low Price</a></li>
                    </ul>
                  </div>
            <div className="row">
              {product?.map((e, i) => {
